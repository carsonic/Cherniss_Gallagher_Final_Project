...
# When using csv data, which I had to do for the shiny app to function properly, parsing the dates is not neccessary, they already show up as dates when you download the data. Attempting to parse time in particular causes the second graph to not render. Be careful using the csv version vs. the excel version. 
```
